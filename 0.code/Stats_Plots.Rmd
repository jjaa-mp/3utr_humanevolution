...
ggplot(final, aes(x=factor(Name, level=order1), y=Normalized))+geom_violin(trim = FALSE)+geom_dotplot(binaxis='y', stackdir='center', aes(fill = factor(Name)), show.legend = FALSE, binpositions = 'all')+ylab("Normalized RLU")+xlab("")+fillScale+theme_classic()+geom_signif(comparisons = list(c("RUNX2-WT", "RUNX2-Mut"), c("SOST-WT", "SOST-Mut")), annotations = c("*", "**"))
```
