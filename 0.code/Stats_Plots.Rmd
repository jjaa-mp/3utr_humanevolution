...
#Reading tables
##Archaic
ensemblhg19_3UTR_w_arc <- read.delim("~/3utr_humanevolution/1.data/1.1.Input_tables/ensemblhg19_3UTR_w_ArchaicSNC.csv")
names(ensemblhg19_3UTR_w_arc) <- c("chr", "3utr_start", "3utr_end", "gene_name")

##Archaic without Homo sapiens
ensemblhg19_3UTR_w_ArchaicSNC_withoutHS <- read.delim("~/3utr_humanevolution/1.data/1.1.Input_tables/ensemblhg19_3UTR_w_ArchaicSNC_withoutHS.csv")
names(ensemblhg19_3UTR_w_ArchaicSNC_withoutHS) <- c("chr", "3utr_start", "3utr_end", "gene_name")

##Homo sapiens
ensemblhg19_3UTR_w_HsapiensSNC <- read.delim("~/3utr_humanevolution/1.data/1.1.Input_tables/ensemblhg19_3UTR_w_HsapiensSNC.csv")
names(ensemblhg19_3UTR_w_HsapiensSNC) <- c("chr", "3utr_start", "3utr_end", "gene_name")


##Homo sapiens without archaics
ensemblhg19_3UTR_w_HsapiensSNC_without_arc <- read.delim("~/3utr_humanevolution/1.data/1.1.Input_tables/ensemblhg19_3UTR_w_HsapiensSNC_without_arc.csv")
...
```{r}
luc1 <- read_excel("~/3utr_humanevolution/1.data/1.1.Input_tables/Luciferase_raw.xlsx", sheet = "GLI, RUNX2, E2F6")
new <-luc1[,c(1, 9)]
new1 <- new[complete.cases(new),]
new1 <- new1[-c(1:9),]
luc2 <- read_excel("~/3utr_humanevolution/1.data/1.1.Input_tables/Luciferase_raw.xlsx", sheet = "SOST")
new2 <-luc2[,c(1, 9)]
...
shapiro.test(x = final$Normalized) #p-value < .01. Data not normally distributed.
#qq plot
ggqqplot(final$Normalized)
#Non-parametric Kruskal-Wallis rank sum test
kruskal.test(Normalized ~ Name, data = final)
#mult comp
pairwise.wilcox.test(final$Normalized, final$Name, p.adjust.method = "BH", confint = TRUE)

#Non parametric: Wilcoxon rank sum test
wilcox.test(final[which(final$Name == "E2F6-WT"),]$Normalized, final[which(final$Name == "E2F6-Mut"),]$Normalized)
wilcox.test(final[which(final$Name == "GLI3-WT"),]$Normalized, final[which(final$Name == "GLI3-Mut"),]$Normalized)
wilcox.test(final[which(final$Name == "RUNX2-WT"),]$Normalized, final[which(final$Name == "RUNX2-Mut"),]$Normalized)
wilcox.test(final[which(final$Name == "SOST-WT"),]$Normalized, final[which(final$Name == "SOST-Mut"),]$Normalized)
```
...
fillScale <- scale_fill_manual(name = unique(final$Name),values = myColors)
order1 <-c('E2F6-WT','E2F6-Mut','GLI3-WT', 'GLI3-Mut','RUNX2-WT','RUNX2-Mut', 'SOST-WT', 'SOST-Mut')

lc <- ggplot(final, aes(x=factor(Name, level=order1), y=Normalized))+geom_violin(trim = FALSE)+geom_dotplot(binaxis='y', stackdir='center', aes(fill = factor(Name)), show.legend = FALSE, binpositions = 'all')+ylab("Normalized RLU")+xlab("")+fillScale+theme_classic()+geom_signif(comparisons = list(c("RUNX2-WT", "RUNX2-Mut"), c("SOST-WT", "SOST-Mut")), annotations = c("*", "**"))
ggsave(file="~/3utr_humanevolution/2.plots/Luc_assay.pdf", lc, width = 11.69, height = 8.27, units = "in")

```

#SOST SNP Genotyping
```{R}
df <- read.csv("~/3utr_humanevolution/1.data/1.3.SOST_BARCOS/SOST_rs17886183_BARCOS.csv")
df <- df[(df1$SNP != "NA"),]
df <- df[(df1$BMD_COL != "NA"),]

nrow(df)

df$BMD_COL <- as.numeric(df$BMD_COL)

df %>%
  group_by(SNP) %>%
  summarise(mean = mean(BMD_COL, na.rm=TRUE))


#Boxplot
df$SNP <- factor(df$SNP, levels = c("GG", "GA"))
pl1 <- ggplot(df, aes(x=SNP, y=BMD_COL, fill=SNP)) + 
  geom_boxplot()+theme_minimal()+ylab("BMD")+xlab("SOST rs17886183")+labs(fill = "Genotype")

ggsave(file="~/3utr_humanevolution/2.plots/SOST_rs17886183.pdf", pl1, width = 11.69, height = 8.27, units = "in")
```